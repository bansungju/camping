...
④ 핫파일(app.js/style.css/build-item-pages.js) 편집 직전 .claude/locks/<파일>.lock 확인:
   ...
   잠금 JSON엔 본인 식별자를 넣는다(이번 회차 SID 정해 사용): echo '{"session":"<SID>","lane":"CORE","file":"app.js"}' > .claude/locks/app.js.lock
   ※ pre-commit이 이제 락을 '강제'한다 — 핫파일 커밋은 LANE_SESSION=<SID>가 락의 session과 일치해야 통과(⑦).
...
⑦ 커밋: 항상 명시적 git add(파일 나열). git add . 금지. git diff --cached --name-only로 제외 파일 미포함 확인.
   핫파일(app.js/style.css/build-item-pages.js)이 포함되면 LANE_SESSION=<④의 SID> git commit -m "..." -- <파일들>.
   (순수 stamp_version/build-item-pages 산출만 담은 커밋은 ALLOW_NO_LOCK=1 git commit 허용.) 커밋·푸시 후 ④ 잠금 삭제.
