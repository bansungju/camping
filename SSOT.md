...
> **이 SSOT는 인프라/배포 사실의 SSOT다. 무엇을 만들지(개발요건)는 여기 두지 않는다.**
> **개발요건(프론트엔드/백엔드 범위로 나눈 기능 리스트)은 [DEV-REQUIREMENTS.md](DEV-REQUIREMENTS.md) 가 단일 진실이다.** 신규/변경 요건은 그 문서에서 관리한다.
