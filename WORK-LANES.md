> # 🛑 루프 일시정지 중 (2026-06-12 21:44 KST~)
> **`.claude/LOOP-PAUSED` 파일이 존재하면 CORE/SOCIAL 버그 루프는 작업 금지·즉시 종료한다.**
> (버그 선택·파일 편집·커밋·푸시 전부 하지 말 것. 이미 돌던 한 사이클은 잠금 해제까지만 마무리 허용.)
> 사유: 인터랙티브 피처 개발 세션과 app.js/style.css 핫파일 충돌 방지. 재개 = `.claude/LOOP-PAUSED` 삭제.
