...
function _execToggleWish(item) {
  ...
  if (added) {
    ...
    if (u && u.id) {
      if (window.Capacitor?.isNativePlatform?.()) {
        import("./supabaseClient.js?v=fc0aca58").then(m => m.registerNativePush && m.registerNativePush()).catch(() => {});
...
async function setupHomeSearch() {
  ...
  const ensureIdx = () => {
    ...
    if (!idxLoading) idxLoading = getJSON("data/search.json?v=54ff954f").then(d => (idx = d)).catch(e => { idxLoading = null; console.warn("search.json load failed", e); return []; });
    return idxLoading;
  };
...
async function setupSearchPage() {
  ...
  const ensureIdx = () => {
    ...
    if (!_idxLoading) _idxLoading = getJSON("data/search.json?v=54ff954f").then(d => (idx = d)).catch(e => { _idxLoading = null; console.warn("search.json load failed", e); return []; });
    return _idxLoading;
  };
...
function openReviewDetail(r) {
  ...
  if (rbtn) rbtn.onclick = async () => {
    ...
    submitBtn2.onclick = async () => {
      ...
      try {
        const { reportContent } = await import("./supabaseClient.js?v=fc0aca58");
...
function wireReviews(modal, m, pcode) {
  ...
  addBtn.onclick = async () => {
    ...
    form.onsubmit = async e => {
      ...
      try {
        const { supabase, getErrorMessage, uploadImage, removeUploadedImages } = await import("./supabaseClient.js?v=fc0aca58");
...
async function renderBrand() {
  renderCatNav("");
  let idx;
  try { idx = await getJSON("data/search.json?v=54ff954f"); }
  catch (e) { document.getElementById("title").textContent = "데이터를 불러오지 못했습니다."; return; }
  const params = new URLSearchParams(location.search);
  ...
  if (logsSec && myLogsList) {
    ...
    if (userId) {
      ...
      if (!myLogsList.dataset.loaded) {
        ...
        import("./supabaseClient.js?v=fc0aca58").then(async ({ supabase, getMyReviews }) => {
          myLogsList.dataset.loaded = "1";
          const logsCnt = document.getElementById("logscount");

          // 후기 → 상품 이동 링크 해석용 인덱스(있으면). 실패해도 후기는 링크 없이 표시.
          let prodMap = new Map();
          try { (await getJSON("data/search.json?v=54ff954f")).forEach(e => prodMap.set(wishKey(e.b, e.m, e.cap), e)); } catch (_) {}
            ...
            postsBox.querySelectorAll(".my-log-edit").forEach(btn => {
              btn.onclick = () => {
                ...
                document.getElementById(`le-save-${pi}`).onclick = async () => {
                  ...
                  let _le;
                  try {
                    const { supabase: sb } = await import("./supabaseClient.js?v=fc0aca58");
    ...
    try {
      ...
      modal.querySelector("#vs-import-btn").onclick = async function() {
        ...
        if (window._accUser?.id) {
          import("./supabaseClient.js?v=fc0aca58").then(async ({ upsertGearSet }) => {
