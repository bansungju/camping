...
### [L-295] ✅ 검토완료·현행유지(2026-06-14, J·코드 실대조) — parse_lumens는 단위(lm|루멘|lumen) 필수 매칭(H-90, 134), 단위 없는 숫자→None. 기수정. — `normalize.py` `parse_lumens` — 단위 없는 임의 숫자를 루멘으로 반환
...
### [L-320] ✅ 검토완료·현행유지(2026-06-14, J·설계) — assert는 if __name__=='__main__' 셀프테스트 블록 전용, 운영은 -O 미사용. 테스트 하니스에 assert 적절. — `normalize.py` `__main__` — `-O` 최적화 모드에서 `assert` 자동 제거
...
### [L-340] ✅ 검토완료·현행유지(2026-06-14, J·데이터) — 혼합단위('30cm x 12inch') 치수 문자열은 다나와/한국 데이터에 미발생(병기 단위 없음), 균일단위 가정이 실데이터에 정확. — `normalize.py` `parse_dims_cm` — 혼합 단위 문자열("cm ... inch")에서 cm 값을 인치로 이중 변환
...
### [L-348] ✅ 검토완료·현행유지(2026-06-14, J·리팩터) — parse_temp는 findall+min()으로 기능 정확(M-319), _num() 미사용은 코드스멜일 뿐 버그 아님. 변경 시 회귀 위험. — `normalize.py` — `parse_temp` 온도 추출 로직 `_num()` 미사용으로 중복 패턴 유지보수 위험
...
### [L-357] ✅ 검토완료·현행유지(2026-06-14, J·도메인) — 캠핑 카탈로그 가격은 항상 ≥1000원(4자리), {4,}가 HTML 노이즈 속 짧은 숫자 오포착 방지. — `danawa.py` `parse_price` — `{4,}` 최소 길이 제약으로 1,000원 미만 가격 미매칭
...
### [L-383] ✅ 검토완료·현행유지(2026-06-14, J·도메인) — 2개 숫자 수납크기는 캠핑 스터프색=원통이 일반적, 작은쪽=지름 휴리스틱 문서화(179). 도메인 적합. — `normalize.py` `packed_volume_cm3` — 2개 숫자 직사각형 치수를 실린더로 오해석 → 부피 과대 계산
...
### [L-404] ✅ 검토완료·현행유지(2026-06-14, J·데이터) — 다나와/한국 데이터는 period 소수+comma 천단위. 유럽식 소수쉼표 미발생, comma 천단위 제거가 정확. — `normalize.py` `parse_dims_cm` — 유럽식 소수 쉼표 문자열 오처리
...
### [L-405] ✅ 해결완료(2026-06-14, J) — parse_capacity_l 정규식에 \b 추가(liter|리터|ℓ|l)\b: 'lb'·'long' 등 오매칭 차단. 8케이스 검증(lb 거부·liter/리터/l 매칭). — `normalize.py` `parse_capacity_l` — 용량 정규식 `l` 단어 경계 없음
...
### [L-406] ✅ 해결완료(2026-06-14, J) — parse_spec_string 빈 키(':값') 가드 추가, 직전 값 연속/태그로 처리(specs[''] 미삽입). 검증: 빈 키 없음 확인. — `danawa.py` `parse_spec_string` — 빈 키 silent 삽입
