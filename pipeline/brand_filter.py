...
BRANDS = {
    ...
    "레드렌서",   # 적대루프9: LED LENSER 오파싱('LED')으로 우연 배제됐던 정상 브랜드 — 동급(오라이트/나이트코어) 이미 등재
    # 버너
    ...
    # ── 적대루프10: 완전성 검토 — 화이트리스트 누락으로 우연 배제됐던 정식 브랜드 일괄 추가 ──
    #    (DB 정확표기 사용. 예: '트란지아(Trangia)'는 기존 '트란지아'와 다른 문자열이라 별도 필요)
    "트란지아(Trangia)", "프리머스", "파이어메이플", "에버뉴", "페트로막스",      # 버너·취사
    "페츨", "에너자이저", "맥라이트", "골제로", "쏠라젠",                         # 조명
    "로고스", "킹캠프", "CHUMS", "헬리콘텍스", "컬럼비아",                        # 캠핑 일반
    "랩(Rab)", "큐물러스", "클라이밋(KLYMIT)", "몬테인", "마모트",               # 침낭·다운
    "도메틱", "샤오미", "365캠퍼",                                            # 쿨러·전원
    "사마야", "HEIMPLANET", "마운틴스미스", "다이와",                          # 텐트·백·기타
    # 적대루프10 2차 전수스캔 추가(고신뢰만 — 인지도 낮은 SME는 해자보호로 보류)
    "발란드레", "발리스틱스", "아이캠퍼", "라디오플라이어", "칸투칸", "콜럼버스코리아", "OtterBox",
}
